<template>
    <div>
        <div class="model_topcol">
            <span>新闻</span>
            <div>
                <el-button type="primary" @click="inAddEvent" size="small">新增新闻</el-button>
            </div>
        </div>
...
export default {
    ...
    methods:{
        ...
        // 获取列表数据
        getTableData(date) {
            api.getnewslist(date).then((response) => {
        ...
        enterTable(index, data) {
            this.$router.push({name: '新闻详情', params: {id: data.id}})
        },
        // 修改
        ...
        deleteTable(index, data) {
            this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                ...
                api.deletenewsdeleteid(data.id).then((response) => {
                    this.$message({
                        message: '删除成功',
                        type: 'success'
                    });
    ...
    activated() {
        ...
        api.getnewslist(this.searchFormData).then((response) => {
            this.tableData = response.data.list
